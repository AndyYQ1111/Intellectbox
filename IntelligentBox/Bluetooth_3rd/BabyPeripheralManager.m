    ...
    return ^BabyPeripheralManager *() {
        ...
        if ([self canStartAdvertising]) {
            ...
            if (self->PERIPHERAL_MANAGER_INIT_WAIT_TIMES > 5) {
                BabyLog(@">>>error： 第%d次等待peripheralManager打开仍然失败，请检查蓝牙设备是否可用",self->PERIPHERAL_MANAGER_INIT_WAIT_TIMES);
                //发送centralManagerDidUpdateState通知
                [[NSNotificationCenter defaultCenter]postNotificationName:@"CBPeripheralManagerStatePoweredOff" object:nil];
